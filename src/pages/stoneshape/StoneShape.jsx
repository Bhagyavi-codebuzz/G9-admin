...
const StoneShape = () => {
    ...
    const filterstoneshape = (stoneshape || []).filter((i) => {
        const searchstr = `${i.name}`.toLowerCase();
        return searchstr.includes(search.toLowerCase());
    });

    const startIndex = (currentPage - 1) * perPage;
    const currentPageData = filterstoneshape.slice(startIndex, startIndex + perPage);
            ...
            (
                <div className="d-flex align-items-center">
                    {/* <button type="button" className="btn btn-sm btn-neutral text-nowrap eye-icon me-3 border"
                        onClick={() => {
                            navigate(`/admin/materials/stoneshape-details/${row?.id}`);
                        }}
                    >
                        <FaEye />
                    </button> */}
                    ...
                    <button type="button" className="btn btn-sm btn-neutral text-nowrap eye-icon me-3 border"
                        onClick={() => {
                            navigate(`/admin/materials/stoneshape-edit`, { state: { stoneshape: row } });
                        }}
                    >
                        <FaEdit />
                    </button>
    ...
    const handleDelete = async () => {
        ...
        try {
            const res = await Axios.delete(apiendpoints.deleteStoneShape.replace(":id", deleteId), authorizationHeaders());
            ...
            if (res.data?.status) {
                ...
                setstoneshape((prev) => prev.filter((i) => i.id !== deleteId));
            }
            else {
                toast.error(res.data?.message);
            }
    ...
    return (
        <>
                            ...
                            <div className="card-header">
                                <div className="d-flex align-items-center justify-content-between flex-wrap gap-3">
                                    ...
                                    <button className="add-btn boreder-0" type="button"
                                        onClick={() => navigate("/admin/materials/addstoneshape")}>
                                        + Add Stone Shape
                                    </button>
                                </div>
                            </div>
...

                                {
                                    error ? (
                                        ...
                                        <DataTableComponents
                                            ...
                                            loader={loader}
                                            filterDataLength={filterstoneshape.length || 0}
            ...
            <Delete show={modalShow.deletestoneshape} handleClose={handleClose} isDeleteLoading={isDeleteLoading} handleDelete={handleDelete} role={"Stone Shape"} />
        </>
    )
}
