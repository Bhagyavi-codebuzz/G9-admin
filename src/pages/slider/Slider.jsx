...
const Slider = () => {
  ...
  const filterslider = (slider || []).filter((i) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        <img
          src={row.slider === null ? UserImage : row.slider}
          alt="Image"
          className={`${row.slider === null && 'rounded-circle'}`}
          style={{
            maxWidth: "100px",
            maxHeight: "100px",
            padding: '8px 0',
  ...
  const fetchSlider = async () => {
    ...
    try {
      const res = await Axios.get(`${apiendpoints.slider}`, authorizationHeaders());
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await Axios.delete(apiendpoints.deleteSlider.replace(":id", deleteId), authorizationHeaders());
      ...
      if (res.data?.status) {
        ...
        setSlider((prev) => prev.filter((i) => i.id !== deleteId));
      }
      else {
        toast.error(res.data?.message);
      }
  ...
  return (
    <>
      ...
      <Delete show={modalShow.deleteslider} handleClose={handleClose} isDeleteLoading={isDeleteLoading} handleDelete={handleDelete} role={"Slider"} />
    </>
  )
}
