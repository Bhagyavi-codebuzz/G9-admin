...
const Offerbar = () => {
  ...
  const filterofferbar = (offerbar || []).filter((i) => {
    const searchstr = `${i.text}`.toLowerCase();
    return searchstr.includes(search.toLowerCase());
  });

  const startIndex = (currentPage - 1) * perPage;
  const currentPageData = filterofferbar.slice(startIndex, startIndex + perPage);
      ...
      (
        <div className="d-flex align-items-center">
          <button type="button" className="btn btn-sm btn-neutral text-nowrap eye-icon me-3 border"
            onClick={() => {
              navigate(`/admin/offerbar-details/${row?.id}`);
            }}
          >
            <FaEye />
          </button>
          ...
          <button type="button" className="btn btn-sm btn-neutral text-nowrap eye-icon me-3 border"
            onClick={() => {
              navigate(`/admin/offerbar-edit`, { state: { offerbar: row } });
            }}
          >
            <FaEdit />
          </button>
  ...
  const fetchOfferbar = async () => {
    ...
    try {
      const res = await Axios.get(`${apiendpoints.offerBar}`, authorizationHeaders());

      if (res.data?.status) {
        setOfferbar(res.data?.data || []);
      }
      else {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await Axios.delete(apiendpoints.deleteOfferbar.replace(":id", deleteId), authorizationHeaders());
      ...
      if (res.data?.status) {
        ...
        setOfferbar((prev) => prev.filter((i) => i.id !== deleteId));
      }
      else {
        toast.error(res.data?.message);
      }
  ...
  return (
    <>
              ...
              <div className="card-header">
                <div className="d-flex align-items-center justify-content-between flex-wrap gap-3">
                  ...
                  <button className="add-btn boreder-0" type="button"
                    onClick={() => navigate("/admin/addofferbar")}>
                    + Add Offer Bar
                  </button>
                </div>
              </div>
...

                {
                  error ? (
                    ...
                    <DataTableComponents
                      ...
                      loader={loader}
                      filterDataLength={filterofferbar.length || 0}
      ...
      <Delete show={modalShow.deleteofferbar} handleClose={handleClose} isDeleteLoading={isDeleteLoading} handleDelete={handleDelete} role={"offerbar"} />
    </>
  )
}
